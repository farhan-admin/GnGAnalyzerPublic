/*
The below code was copied from the SegYIO segy.c file
downloaded from https://github.com/equinor/segyio
 And Madagascar SegY reader
*/

#ifndef CSEGYFUNC_H
#define CSEGYFUNC_H

#include <QString>

using namespace std;

//namespace GnV //GnGViewer
//{

#define SF_SEGY_FORMAT  24
#define SF_SEGY_NS      20
#define SF_SEGY_DT      16



    typedef unsigned char byte;

    constexpr static byte EBCtoASC[256] = {
        0x00,0x01,0x02,0x03,0xCF,0x09,0xD3,0x7F,
        0xD4,0xD5,0xC3,0x0B,0x0C,0x0D,0x0E,0x0F,
        0x10,0x11,0x12,0x13,0xC7,0xB4,0x08,0xC9,
        0x18,0x19,0xCC,0xCD,0x83,0x1D,0xD2,0x1F,
        0x20,0x82,0x1C,0x84,0x86,0x0A,0x17,0x1B,//Farhan: changed, value at position 32, from 0x81 to 0x20
        0x89,0x91,0x92,0x95,0xA2,0x05,0x06,0x07,//because 0x81, is undefined in ASCII or Extended ASCII tables
        
    };


    class CSegYFunc
    {
    public:
        CSegYFunc();

        bool little_endian;
        enum {
            SF_EBCBYTES = 3200,	/* Bytes in the card image EBCDIC block */
            SF_BNYBYTES = 400,	/* Bytes in the binary coded block	*/
            SF_HDRBYTES = 240,	/* Bytes in the tape trace header	*/
            SF_NKEYS = 91,	/* Number of mandated header fields	*/
            SF_BHKEYS = 27,	/* Number of mandated binary fields	*/
            SF_MAXKEYS = 256      /* Maximum number of keys               */
        };

        //Check Endiness
        bool is_little_endian();


        static void native_ibm(void* buf);
        static uint16_t htobe16(uint16_t v);
        static uint32_t be32toh(uint32_t v);
        void ibm2ieee(void* to, const void* from);
        void ibm2float(float* y, const char* num);
        static void ibm_native(void* buf);

        void ebc2asc(int narr, char* arr);
        qint32 convert4(const char* buf);
        qint16 convert2(const char* buf);

    };
//}
#endif // CSEGYFUNC_H
