void cGLWindow::RenderSeisGath(QOpenGLShaderProgram* program, QMatrix4x4 matCrdScl)
{
    //Cycle through all header objects
    quint32 nObjs = m_LstSeisXZYGths.size();
    ...
    for (auto nObj : m_LstSeisXZYGths)
    {
        std::shared_ptr<QVector<qreal>> vTrc = nObj->m_Traces; ////CHANGED
        ...
        for (uint i = 0; (i + 2 + nObj->NumofSamples) < vTrc->size(); i = i + 3 + nObj->NumofSamples)
        {
            matModel.setToIdentity();

            matModel.translate(QVector3D(   nObj->m_Traces->at(i),  //X value
                                            nObj->m_Traces->at(i + 1), //Z value
                                            nObj->m_Traces->at(i + 2)) // Y value
                                            * matCrdScl); // Translation Matrix

            qDebug() << "Gather model" << matModel;
            ...
            program->setUniformValue(m_ColorUniform, QVector4D(1.0, 1.0, 1.0, 1.0));
